...
\begin{proof}
	...
	\begin{enumerate}[label = \textit{Step \arabic*:},wide=0pt]
		...
		\item \textit{Existence and Uniqueness of Weak Solutions.} Since $L$ is uniformly elliptic, there exists $\lambda > 0$ such that 
			...
			\end{equation*}
			\noindent holds for almost all $x \in \Omega$ and $\xi,\eta \in \mathbb{R}^n$. Now define a bilinear form $\langle \cdot,\cdot \rangle_a : H^1_0(\Omega) \times H^1_0(\Omega) \to \mathbb{R}$ by
			\begin{equation}
				\label{eq:inner_prod}
...
\begin{theorem}
	Let $\Omega \subseteq \subseteq \mathbb{R}^n$ connected and of class $C^1$ and $1 \leq p < \infty$. Then 
	\begin{equation*}
		\norm[0]{u - \wbar{u}}_{L^p(\Omega)}^p \leq C \norm[0]{\nabla u}_{L^p(\Omega)}^p.
	\end{equation*}
\end{theorem}
...
\input{regularity_theory.tex}
\input{Schauder_theory.tex}
\input{maximum_principle.tex}
