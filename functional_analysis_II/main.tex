...
\begin{examples}[Elliptic Operators in Divergence Form]
	~
	\begin{enumerate}[label = \textup{(}\alph*\textup{)},wide = 0pt]
		\item Set $a^{ij}(x) := \delta^{ij}$ for all $x \in \Omega$. Then $A_0 = - \Delta$. Moreover $A_0$ is uniformly elliptic, since $\delta^{ij}\xi_i\xi_j = \abs{\xi}^2$ for all $\xi \in \mathbb{R}^n$.
