...
\begin{definition}[Category of Left $\mathfrak{g}$-Modules]
	...
	\begin{equation*}
		[x,y]m = x(ym) - y(xm)
	\end{equation*}
	\noindent holds for all $x,y \in \mathfrak{g}$ and $m \in M$, and \bld{left $\mathfrak{g}$-module homomorphisms} as morphisms, i.e. morphisms $f \in \Hom_{_{R}\mathsf{Mod}}(M,N)$ such that 
	\begin{equation*}
		f(xm) = xf(m)
	\end{equation*}
